            ...
            if showsChrome {
                ...
                GeometryReader { layout in
                    HStack(spacing: 0) {
                        ...
                        let sidebarWidth = sidebarShouldShow
                            ? min(browser.sidebarDragWidth ?? settings.sidebarWidth, browser.maxAllowedSidebarWidth)
                            : 0
                        let resizeHandleWidth = sidebarShouldShow ? max(10, settings.windowMargin) : 0
                        let availablePageWidth = max(0, layout.size.width - sidebarWidth - resizeHandleWidth)
                        let pageWidth = sidebarShouldShow
                            ? min(availablePageWidth, availablePageWidth * settings.webpageWidth)
                            : layout.size.width
