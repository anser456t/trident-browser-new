...
final class ExtensionAPIBridge: NSObject, WKScriptMessageHandler {
    ...
    private var registeredMessageListeners = Set<String>()
    ...
    private func handle(extensionID: String, api: String, method: String, args: [String: Any],
                         sourceWebView: WKWebView, sourceWorld: WKContentWorld,
        ...
        case ("runtime", "addMessageListener"):
            if !registeredMessageListeners.contains(extensionID) {
                registeredMessageListeners.insert(extensionID)
                ExtensionMessageBroker.shared.addListener(extensionID) { [weak sourceWebView] message, senderContext in
                    guard let sourceWebView,
                          ...
                          let senderData = try? JSONSerialization.data(withJSONObject: ["context": senderContext]),
                          let senderJSON = String(data: senderData, encoding: .utf8) else { return }
                    DispatchQueue.main.async {
                        sourceWebView.evaluateJavaScript(
                            "window.__tridentDispatchMessage && window.__tridentDispatchMessage(\(messageJSON), \(senderJSON));",
                            in: nil,
                            in: sourceWorld
                        ) { _ in }
                    }
        ...
        case ("tabs", "get"):
            ...
            guard let browser, let idString = args["tabId"] as? String, let uuid = UUID(uuidString: idString),
                  let tab = browser.tabs.first(where: { $0.id == uuid && !$0.isPrivate }) else { return completion(.failure(.notFound)) }
    ...
    nonisolated static func shimScript(extensionID: String) -> String {
        ...
        (function() {
          ...
          var messageListeners = [];
           window.__tridentDispatchMessage = function(message, sender) {
             messageListeners.slice().forEach(function(fn) {
               try { fn(message, sender, function() {}); } catch (e) { console.error(e); }
             });
           };
            ...
            onMessage: {
               addListener: function(fn) {
                 if (typeof fn === 'function') {
                   ...
                   call('runtime', 'addMessageListener', {});
                 }
               }
            }
          };
