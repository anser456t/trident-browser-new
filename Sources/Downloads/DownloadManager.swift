...
final class DownloadManager: ObservableObject {
    ...
    private var historyRetained = true
    ...
    func setHistoryRetained(_ retained: Bool) {
        historyRetained = retained
        guard !retained, let context = modelContext else { return }
        let descriptor = FetchDescriptor<DownloadItem>()
        let items = (try? context.fetch(descriptor)) ?? []
        items.filter { $0.status != .inProgress }.forEach { context.delete($0) }
        try? context.save()
    }
    ...
    func markCompleted(id: UUID) {
        ...
        if !historyRetained { removeRecord(id: id) }
    }
    ...
    func registerCancellationHandler(for id: UUID, handler: @escaping () -> Void) {
        cancellationHandlers[id] = handler
    }

    func clearCancellationHandler(for id: UUID) {
        cancellationHandlers[id] = nil
    }

    /// Cancels the native transfer before the UI removes its persistent row.
    /// The handler is intentionally supplied by WebViewController so this
    /// manager does not need to own or retain WKDownload instances.
    func cancel(id: UUID) {
