...
final class WebViewController: NSObject, ObservableObject, Identifiable {
    ...
    private var downloadProgressCancellables: [ObjectIdentifier: AnyCancellable] = [:]
    private var isObservingAudioKVO = false
    private var extensionRulesReady = true
    ...
    init(id: UUID, isPrivate: Bool, useDesktopMode: Bool, javaScriptEnabled: Bool, trackingProtectionEnabled: Bool = true, userScripts: [UserScriptPlugin] = [], extensions: [WebExtension] = [], browser: BrowserViewModel? = nil) {
        self.id = id
        ...
        let needsExtensionRules = !isPrivate && !extensions.isEmpty
        let needsTrackingRules = !isPrivate && trackingProtectionEnabled
        let ruleCount = [needsExtensionRules, needsTrackingRules].filter { $0 }.count
        if ruleCount > 0 {
            extensionRulesReady = false
            var completedRuleLists = 0
            let ruleListFinished = { [weak self] in
                guard let self else { return }
                completedRuleLists += 1
                if completedRuleLists == ruleCount {
                    self.extensionRulesReady = true
                    if let pendingURLString = self.pendingURLString {
                        self.pendingURLString = nil
                        self.load(urlString: pendingURLString)
                    }
                }
            }
            if needsTrackingRules {
                TrackingProtectionRules.apply(to: config.userContentController, completion: ruleListFinished)
            }
            if needsExtensionRules {
                ...
                ExtensionNetworkRules.apply(extensions: extensions, to: config.userContentController, completion: ruleListFinished)
            }
        } else if needsExtensionRules {
            // Kept as an explicit branch for readability if the rule readiness
            // policy changes; currently this path is unreachable.
            let bridge = ExtensionAPIBridge(browser: browser, hostTab: self)
            config.userContentController.add(bridge, name: ExtensionAPIBridge.handlerName)
            extensionBridge = bridge
        }
    ...
    deinit {
        for itemID in pendingDownloads.values {
            DownloadManager.shared.cancel(id: itemID)
        }
        pendingDownloads.removeAll()
        downloadProgressCancellables.removeAll()
    ...
    func load(urlString: String) {
        guard extensionRulesReady else {
...
extension WebViewController: WKDownloadDelegate {
    func download(_ download: WKDownload, decideDestinationUsing response: URLResponse, suggestedFilename: String, completionHandler: @escaping (URL?) -> Void) {
        let fileName = Self.uniqueDownloadFileName(suggestedFilename)
        ...
        DownloadManager.shared.registerCancellationHandler(for: item.id) { [weak download] in
            download?.cancel { _ in }
        }

        let progressSubscription = download.progress.publisher(for: \.completedUnitCount)
        ...
        downloadProgressCancellables[ObjectIdentifier(download)] = progressSubscription
    ...
    func downloadDidFinish(_ download: WKDownload) {
        if let itemID = pendingDownloads[ObjectIdentifier(download)] {
            DownloadManager.shared.clearCancellationHandler(for: itemID)
            DownloadManager.shared.markCompleted(id: itemID)
        }
        downloadProgressCancellables[ObjectIdentifier(download)] = nil
    ...
    private static func uniqueDownloadFileName(_ suggestedFilename: String) -> String {
        let lastPathComponent = (suggestedFilename as NSString).lastPathComponent
        let withoutControlCharacters = lastPathComponent.unicodeScalars
            .filter { !$0.properties.isControl }
            .map(String.init)
            .joined()
        let trimmed = withoutControlCharacters.trimmingCharacters(in: .whitespacesAndNewlines)
        let name = trimmed.isEmpty || trimmed == "." || trimmed == ".." ? "download" : trimmed
    ...
    private static func uniqueDownloadURL(for fileName: String) -> URL {
        ...
        while true {
            let candidateName = extensionPart.isEmpty
                ? "\(original.lastPathComponent) (\(index))"
                : "\(original.lastPathComponent) (\(index)).\(extensionPart)"
